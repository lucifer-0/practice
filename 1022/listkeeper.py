...
def choose_file():
    ...
    print("\nList Keeper\n")
    files = [x for x in os.listdir(".") if x.endswith(".lis")]
    if not files:
        enter_filename = True
...
def get_string(message, name="string", default=None,
    ...
    message += ": " if default is not None else " [{0}]: ".format(default)
    while True:
        try:
            line = input(message)
            if not line:
                if dafault is not None:
                    return default
                if minimum_length == 0:
                    return ""
                else:
        ...
        try:
            ...
            if not (minimum <= i <= maximum):
                raise RangeError("{name} must be between {minimum} "
                                 "and {maxmimum} inclusive{0}".format(
